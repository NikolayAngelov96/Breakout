...
export function generateBricks(ctx: CanvasRenderingContext2D, level: number) {
    ...
    for (let row = 0; row < levels[level].rows; row++) {
      for (let col = 0; col < levels[level].cols; col++) {
        ...
        if (row == 0) {
          bricks.push(new Brick(ctx, brickX, brickY, brickWidth, brickHeight, 3, true));
        } else if (row == 1) {
          bricks.push(new Brick(ctx, brickX, brickY, brickWidth, brickHeight, 2, true));
        } else if (row == 2) {
          bricks.push(new Brick(ctx, brickX, brickY, brickWidth, brickHeight, 1, true));
        } else if (row == 3) {
            if (col % 2 == 0) {
                bricks.push(new Brick(ctx, brickX, brickY, brickWidth, brickHeight, 4, true));
            } else {
                if (level == 3) {
                    bricks.push(new Brick(ctx, brickX, brickY, brickWidth, brickHeight, 5, true));
                }
            }
        }
      }
